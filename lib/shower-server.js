...
exports.initExpressStatic = function (presentations, app, dirname) {
    ...
    app.get('/client.js', function (req, res) {
        res.sendFile(clientJsFile, { root: path.join(__dirname, '../') });
    });
    ...
    presentations.forEach(function (presentation) {
        ...
        app.get(url, function (req, res) {
            var transformHeadStream = new stream.Transform(),
                transformBodyStream = new stream.Transform(),
                _transformGenerator = function (regexp, replacement) {
            ...
            transformHeadStream._transform = _transformGenerator(/<\/head>/, '<script src="/socket.io/socket.io.js"></script></head>');
            transformBodyStream._transform = _transformGenerator(/<\/body>/, '<script type="text/javascript" src="/client.js"></script></body>');

            transformHeadStream._flush = transformBodyStream._flush = function (done) {
            ...
            try {
                utils.readFile(dirname + '/' + folder + '/' + file)
                    .pipe(transformHeadStream)
                    .pipe(transformBodyStream)
                    .pipe(res);
            } catch (e) {
                res
                    .status(500)
                    .send('Whoops, could not modify presentation file...');
            }
        });
